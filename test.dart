...
class FaceRecognitionApp:
    def __init__(self, root):
        ...
        self.keyboard_settings = {
            ...
            'add_tab': False,
            'delay_seconds': 3,
            'format_template': '{name}',  # Can use {name}, {timestamp}, {date}, {time}
            'target_window': ''  # Optional: specific window to target
        }
    ...
    def load_known_faces(self):
        """Load known faces from the known_people folder with subfolders"""
        known_people_folder = "./known_people"
        if not os.path.exists(known_people_folder):
            os.makedirs(known_people_folder)
            messagebox.showwarning("Warning", f"Created '{known_people_folder}' folder. Please add subfolders with up to 10 images each.")
            return
        
        self.known_encodings = []
        self.known_names = []
        
        print(f"Debug: Checking face_recognition module: {dir(face_reco)}")  # Debug print
        if not hasattr(face_reco, 'load_image_file'):
            print("Error: load_image_file not found in face_recognition module. Installation may be corrupted.")
            return
        ...
        for person_folder in os.listdir(known_people_folder):
            ...
            if os.path.isdir(person_path):
                ...
                if not image_files:
                    print(f"Warning: No images found in {person_folder}")
                    continue
                ...
                for i, filename in enumerate(image_files):
                    ...
                    try:
                        image = face_reco.load_image_file(image_path)
                        encodings = face_reco.face_encodings(image)
                        if encodings:
                            for j, encoding in enumerate(encodings):
                                self.known_encodings.append(encoding)
                                name = person_folder
                                if len(encodings) > 1:
                                    name = f"{person_folder}_{j+1}"
                                self.known_names.append(name)
                                print(f"Loaded face: {name} from {filename}")
                        else:
                            print(f"Warning: No face detected in {filename} in {person_folder}")
                    except Exception as e:
                        print(f"Error loading {filename} in {person_folder}: {e}")
    ...
    def upload_image(self):
        ...
        try:
            ...
            image = face_reco.load_image_file(file_path)
            frame = cv2.imread(file_path)
            if frame is None:
                messagebox.showerror("Error", "Failed to load image")
                return
            
            # Stop camera if running to show static image
            if self.is_camera_running:
                self.stop_camera()
            
            # Process image for face recognition
            rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            self.face_locations = face_reco.face_locations(rgb_frame, model="hog", number_of_times_to_upsample=1)
            self.face_encodings = face_reco.face_encodings(rgb_frame, self.face_locations)
            
            self.current_faces_count = len(self.face_locations)
            self.face_names = []
            
            for face_encoding in self.face_encodings:
                matches = face_reco.compare_faces(self.known_encodings, face_encoding, tolerance=0.5)
                name = "Unknown"
                confidence = 0
                
                if True in matches:
                    face_distances = face_reco.face_distance(self.known_encodings, face_encoding)
            ...
            # Resize image to fit display if necessary
            ...
            self.update_status()
            messagebox.showinfo("Success", f"Processed image: {self.current_faces_count} faces detected")
    ...
    def update_frame(self):
        ...
        if ret:
            ...
            if self.frame_count % 3 == 0:
                ...
                self.face_locations = face_reco.face_locations(rgb_small_frame, 
                                                                    model="hog", 
                                                                    number_of_times_to_upsample=1)
                self.face_encodings = face_reco.face_encodings(rgb_small_frame, self.face_locations)
...
if __name__ == "__main__":
    ...
    # Install face_recognition if not present
    try:
        import face_reco
    except ImportError:
        print("Installing face_recognition and its dependencies...")
        import subprocess
        import sys
